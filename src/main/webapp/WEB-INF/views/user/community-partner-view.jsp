...
<body class="animsition restyle-index">
    ...
    <div class="sub_container in_1400">
		...
		<main>
            ...
            <div class="container_txt container_wrap" style="border-top:none; padding-top:0;">
                <!-- <div class="board_label"data-post-element="level"></div> -->
	                                ...
	                                <div class="talk2">
	                                    ...
	                                    <div class="talk2_btns">
	                                       ...
	                                       <sec:authorize ifAnyGranted="ROLE_USER">
				                                <c:if test="${post.writerId eq VIEWER_ID}">
            ...
            <div class="page_btn_wrap">
            	<div class="back_btn">
            		<a href="<c:url value="/user/community/list" />">목록</a>
            	</div>
            	...
            	<sec:authorize ifAnyGranted="ROLE_USER">
            		<c:if test="${WRITER_ID eq VIEWER_ID}">
		            	<div class="function_btns">
							<a href="<c:url value="/user/community/edit/form/" />${SEQ_ID}" class="modify_btn">수정</a>
		                    <a href="#" class="delet_btn" onclick="deleteNoticePost(${SEQ_ID})">삭제</a>
		                </div>
	                </c:if>
            	</sec:authorize>
            ...
            <!-- 
            <div class="back_btn"><a href="<c:url value="/user/community/list" />">목록</a></div>
			...
			function deleteCommentPost(seqId) {
				if(confirm("해당 작성하신 댓글을 삭제하시겠습니까?")) {
					$.ajax({
						type : 'POST',
						cache: false,
						url : '<c:url value="/user/community/comment/post/remove" />/' + seqId,
						headers : {
							'${_csrf.headerName}' : '${_csrf.token}'
						},
						success : function(result) {
				...
				function deleteNoticePost(seqId) {
					if(confirm("해당 게시물을 삭제하시겠습니까?")) {
						$.ajax({
							type : 'POST',
							cache: false,
							url : '<c:url value="/user/community/post/remove" />/' + seqId,
							headers : {
								'${_csrf.headerName}' : '${_csrf.token}'
							},
							success : function(result) {
