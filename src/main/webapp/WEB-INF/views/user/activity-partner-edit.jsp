	...
	<script >
		$(function() {	// DOM init
			...
			CKEDITOR.replace('post_editor', {
				height: 450,
				filebrowserUploadUrl: '<c:url value="/user/partner-activity/upload/image" />',
		...
		function editPost() {
			...
			if($('#file').val()){
				...
				$.ajax({
					...
					url: '<c:url value="/user/partner-activity/upload/file" />', 
					headers : {
			...
			if(confirm('게시글을 수정하시겠습니까?')) {
				$.ajax({
					type : 'POST',
					cache: false,
					url : '<c:url value="/user/partner/activity/post/edit" />/${SEQ_ID}',
					headers : {
						'${_csrf.headerName}' : '${_csrf.token}'
					},
					processData : false,
					...
					success : function(result) {
						if(result.count > 0) {
							...
							location.href = '<c:url value="/partner/activity/view" />/${SEQ_ID}';	
						} else {
