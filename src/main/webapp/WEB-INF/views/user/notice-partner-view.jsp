    ...
    <div class="sub_container in_1400">
		...
		<main>
            ...
            <div class="container_txt">
                ...
                <ul>
                    <li>작성자<span data-post-element="contractStartAt">${WRITER_NAME}</span></li>
            ...
            <div class="back_btn"><a href="<c:url value="/user/partner/notice/list" />">목록</a></div>
			<!-- 
			<c:choose>
				<c:when test="${IS_AUTHOR == true }">
					<button type="button" class="search-btn mg-top-30" onclick="location.href='<c:url value="/user/partner/notice/edit/form/" />${SEQ_ID}';" style="margin-right: 10px;">수정</button>
		    	...
		    	function deleteNoticePost(seqId) {
					if(confirm("해당 게시물을 삭제하시겠습니까?")) {
						$.ajax({
							type : 'POST',
							cache: false,
							url : '<c:url value="/user/partner/notice/post/remove" />/' + seqId,
							headers : {
							...
							success : function(result) {
								if(result.count > 0) {
									...
									location.href = '<c:url value="/user/partner/notice/list" />';	
								} else {
