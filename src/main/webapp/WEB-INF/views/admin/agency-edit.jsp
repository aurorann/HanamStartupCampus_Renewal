...
<body class="animsition restyle-index">
            ...
            <div class="right-contents col-lg-9">
                    ...
                    <div class="col-lg-12 r-contents">
                    	...
                    	<div class="col-lg-12" style="margin:0; padding:0;">
	                        <h3 class="m-title"><img src="<c:url value='/resources/img/sub-title.png' />">&nbsp;&nbsp;바이오 정책 · 지원 수정</h3>
	                    </div>
	...
	<script >
		...
		function editPost() {
			...
			var agencyName = document.querySelector('#agency_name').value;
			...
			if(title.trim() === '' || title.length > 50) {
				...
				return false;
			} else if(typeof agencyName === "undefined" ||agencyName === '') {
				alert('기관명을 작성해주세요.');
				return false;
			} else if(agencyName.length > 30) {
				alert('기관명을 30자 이내로 입력해주세요.');
				return false;
			}
			// Validation end
			...
			if(confirm('게시글을 수정하시겠습니까?')) {
				$.ajax({
					type : 'POST',
					cache : false,
					url : '<c:url value="/admin/announce/agency/post/edit" />/${SEQ_ID}',
					headers : {
						'${_csrf.headerName}' : '${_csrf.token}'
					},
					data: data,
					success : function(result) {
						if(result.count > 0) {
							...
							location.href = '<c:url value="/announce/agency/view" />/${SEQ_ID}';	
						} else {
