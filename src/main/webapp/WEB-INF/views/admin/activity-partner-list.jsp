...
<body class="animsition restyle-index">
			...
			<!-- # include: title-box end -->
				...
				<div class="container_wrap">
				    ...
				    <div class="btn-group btn-group-justified" role="group" aria-label="Justified button group"  style="padding-bottom: 20px;">
						...
						<a href="javascript:void(0);" class="btn btn-default you-are-here" role="button">기업 소식</a>
                    ...
                    <div class="search_wrap">
	                    ...
	                    <form class="search_form" id="searchForm" action="<c:url value='/admin/partner/activity/list' />" method="get" action="#" onsubmit="return searchNoticeList(this)">
	                ...
	                <table class="board_wrap">
	                    ...
	                    <tbody class="board_body activity-list">
	                    	<!-- 
	                        <tr>
	                ...
	                <!-- 
					<button type="submit" class="" onclick="location.href='<c:url value="/admin/partner/activity/write/form" />';" style="margin-top: 20px;">글쓰기</button>
					 -->
                    <div class="pagination_wrap">
					</div>
	                <a href='<c:url value="/admin/partner/activity/write/form" />' class="write_btn">글쓰기</a>
                </div>
        </main>
    </div>
	<!-- footer start -->
	...
	<script >
		...
		function getPostList(curPage) {
			$.ajax({
				type : 'GET',
				url : '<c:url value="/admin/partner/activity/post/list" />',
				data: {
				...
				success : function(res) {
					...
					commonPost.every(function(el) {
						var titleString = (el.title.length > 28) ?
								...
								var commonHTML = '<tr>'
									...
									+ '<td class="board_title">'
									+ '<a href="<c:url value='/admin/partner/activity/view' />/' + el.seqId + '">' + titleString + '</a>'
									+ '</td>'
					...
					$("tbody.board_body.activity-list").html(commonArr.join(""));
		...
		$(document).on('change','input[id="viewApprove"]', function(){
			...
			//alert(seqId);
			 if ($(this).is(':checked')) {
				 //alert("체크");
				 viewApprove = 'Y';
				 updateApprove(viewApprove, seqId)
			}else{
				//alert("체크해제");
				viewApprove = 'N';
				updateApprove(viewApprove, seqId)
		...
		function updateApprove(viewApprove, seqId) {
			$.ajax({
				type : 'POST',
				cache: false,
				url : '<c:url value="/admin/partner/activity/edit/approve" />',
				headers : {
				...
				error : function(error) {
					alert('게시허용 수정을 못했습니다, 관리자에게 문의해주세요.');
				}
			});
		}
