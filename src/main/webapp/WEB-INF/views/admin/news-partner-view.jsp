		...
		<main>
            <div class="right-contents col-lg-9">
                ...
                <div class="col-lg-12 r-contents">
                            ...
                            <tr>
                                <td>
                                    <div class="board-title">
                                    	<span class="white-label">기업 소식</span>
                    ...
                    <button type="button" class="search-btn mg-top-30" onclick="location.href='<c:url value="/admin/partner/news/list" />'" style="margin-right: 10px;">목록</button>
                    <sec:authorize ifAnyGranted="ROLE_ADMIN">
						<button type="button" class="search-btn mg-top-30" onclick="location.href='<c:url value="/admin/partner/news/edit/form/" />${SEQ_ID}';" style="margin-right: 10px;">수정</button>
					...
					<sec:authorize ifAnyGranted="ROLE_USER">
						<c:if test="${WRITER_ID eq VIEWER_ID}">
							<button type="button" class="search-btn mg-top-30" onclick="location.href='<c:url value="/user/partner/news/edit/form/" />${SEQ_ID}';" style="margin-right: 10px;">수정</button>
							<button type="button" class="search-btn mg-top-30" onclick="deleteNoticePost(${SEQ_ID});" style="margin-right: 10px;">삭제</button>
						</c:if>
						<input type='hidden' id="representImage" value='${REPRESENT_IMAGE}' />
					</sec:authorize>
			...
			function deleteNoticePost(seqId) {
				if(confirm("해당 게시물을 삭제하시겠습니까?")) {
					$.ajax({
						type : 'POST',
						cache: false,
						url : '<c:url value="/admin/partner/news/post/remove" />/' + seqId,
						headers : {
						...
						success : function(result) {
							if(result.count > 0) {
								...
								location.href = '<c:url value="/partner/news/list" />';	
							} else {
